...
import { add__OUR_USER } from '../../../store/actions/user/adminUsers/adminUsers';
...
const UserAdminAdd = ({
  ...
  const onSubmit = async (e) => {
    ...
    add__OUR_USER(name, email, role, password);
    history.goBack();
  };
              ...
              >
                {roles.map((item) => (
                  <MenuItem key={item} value={item}>
                    <Checkbox checked={role.indexOf(item) > -1} />
                    {item}
                  </MenuItem>
                ))}
              </Select>
...
UserAdminAdd.propTypes = {
  ...
  add__OUR_USER: PropTypes.func.isRequired,
};
